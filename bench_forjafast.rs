...
use forja::vm_jit::{compilar_bytecode, BytecodeDT, ForjaDT};
...
fn print_header(titulo: &str) {
    ...
    println!(
        "  {:<22} {:>14} {:>14} {:>14} {:>14}",
        "VM", "Cold (μs)", "Hot (μs)", "vs Original", "vs Rust"
    );
    print_separador();
}
...
fn print_fila(nombre: &str, cold: f64, hot: f64, baseline: f64, rust_us: f64, highlight: bool) {
    ...
    let emoji = if highlight {
        if ratio_vm >= 5.0 {
            " 🏆⚡⚡"
        } else if ratio_vm >= 3.0 {
            " 🏆⚡"
        } else if ratio_vm >= 2.0 {
            " 🏆"
        } else {
            ""
        }
    ...
    println!(
        "  {:<22} {:>12.2} {:>12.2} {:>12.2}x {:>10.1}x{}",
        nombre, cold, hot, ratio_vm, ratio_rust, emoji
    );
}
...
fn main() {
    ...
    println!(
        "  ║     {} warmup · {} hot iters · reusando VM                    ║",
        WARMUP_ITERS, HOT_ITERS
    );
    ...
    let bcs: Vec<Vec<Opcode>> = tests
        .iter()
        .map(|(_, src, _)| compilar_optimizado(src))
        .collect();
    ...
    for (test_idx, (test_name, _, _)) in tests.iter().enumerate() {
        ...
        // Mostrar speedups
        let fast_vs_orig = if hot_orig > 0.0 {
            hot_orig / hot_fast
        } else {
            0.0
        };
        let fast_vs_jit = if hot_jit > 0.0 {
            hot_jit / hot_fast
        } else {
            0.0
        };
        println!(
            "  🏆 ForjaFast vs Original: {:.2}x · vs JIT: {:.2}x",
            fast_vs_orig, fast_vs_jit
        );

        if hot_fast > 0.0 {
            let cold_hot_ratio = cold_fast / hot_fast;
            println!(
                "  🌡️  ForjaFast cold vs hot: {:.2}x (setup + warmup overhead)",
                cold_hot_ratio
            );
        }
    }
    ...
    let avg_jr = if avg_orig > 0.0 {
        avg_orig / avg_jit
    } else {
        0.0
    };
    let avg_fr = if avg_orig > 0.0 {
        avg_orig / avg_fast
    } else {
        0.0
    };
    ...
    if avg_fast > 0.0 {
        println!(
            "  🏆 ForjaFast es {:.1}x más rápido que Original (hot)",
            avg_fr
        );
        println!(
            "  🏆 ForjaFast es {:.1}x más rápido que JIT (DT)",
            avg_orig / avg_fast
        );
    }
    ...
    println!();
    let cold_ratios: Vec<f64> = (0..tests.len())
        .map(|i| {
    ...
    println!(
        "  🌡️  Cold/Hot ratio promedio (ForjaFast): {:.1}x",
        avg_cold_hot
    );
