...
use forja::bytecode::{fusionar_opcodes, optimizar_indices, BytecodeGenerator, Opcode};
...
escribir(s)"#,
        ...
        (
            "condicional 5>3",
            r#"si (5 > 3) { escribir("verdadero") } sino { escribir("falso") }"#,
        ),
        (
            "variables y suma",
            r#"variable x = 5
...
"#;
    ...
    match vm_rec.ejecutar() {
        Ok(_) => println!(
            "  Original VM (unopt): OK! output: {:?}",
            vm_rec.obtener_output()
        ),
    ...
    match vm_rec2.ejecutar() {
        Ok(_) => println!(
            "  Original VM (fused): OK! output: {:?}",
            vm_rec2.obtener_output()
        ),
